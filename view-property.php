...
if(isset($_POST['cancel_booked'])) {
            $propertyid = $_POST['property_id'];
			 $ownerid = $_POST['owner_id'];
             $u_email=$_SESSION["email"];
$sql1 = "UPDATE add_property SET booked='No' WHERE property_id ='$propertyid'";
...
$sql="SELECT * FROM tenant where email='$u_email'";
    ...
    $sql2="DELETE FROM booking where property_id='$propertyid' AND tenant_id='$tenant_id'";

    $query2 = mysqli_query($db,$sql2);
	  }
	}
if(mysqli_query($db, $sql1)){
    echo "Property was updated successfully.";
} else {
      ...
      <div class="col-sm-6">
          <table>
            ...
            <tr>
              ...
              <td><h4>BDT.<?php echo $rows['estimated_price']; ?></h4></td>
            </tr>
          </table>
        </div>
      </div>
...
<div class="row">
  <div class="col-sm-6">
    ...
    if ($booked=='Yes'){ ?>
      <input type="hidden" name="property_id" value="<?php echo $rows['property_id']; ?>">
    <input type="submit" class="btn btn-lg btn-primary" name="book_property" style="width: 100%" value="Payment">
  <?php } else { ?>
    <input type="submit" class="btn btn-lg btn-primary" style="width: 100%" value="Payment Not Available" disabled>
  <?php } ?>
  </div>
</form>
<form method="POST">
  ...
  <input type="hidden" name="property_id" value="<?php echo $rows['property_id']; ?>">
  <input type="hidden" name="owner_id" value="<?php echo $rows['owner_id']; ?>">
    <input type="submit" class="btn btn-lg btn-success" name="cancel_booked" style="width: 100%" value="Cancel Booked" >
    <?php } else { ?>
	
	    <input type="submit" class="btn btn-lg btn-success" style="width: 100%" value="Cancel Not Available" disabled>
  <?php } ?>
  </div>
  </form>
</div>
